...
use crate::{
    database::repository::{DocumentRepository, RepositoryError, StorageConnection},
    domain::{
        document::{Document, RawDocument},
        document_common_ancestor::{
            common_ancestors, AncestorDB, AncestorDetail, CommonAncestorError, InMemoryAncestorDB,
        },
        document_merge::{three_way_merge, two_way_merge, TakeLatestConflictSolver},
    },
};
...
mod document_service_test {
    ...
    async fn test_insert_and_auto_resolve_conflict() {
        let settings = test_db::get_test_settings("omsupply-database-document_service");
